...
export interface SendArchitectureQuoteDialogData {
  requestId: number;
}

@Component({
  selector: 'app-send-architecture-quote-dialog',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './send-architecture-quote-dialog.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SendArchitectureQuoteDialogComponent {
  saving = false;
  serverErrors: Record<string, string[]> = {};

  form = this.fb.group({
    total_amount_mt: [null as number | null, [Validators.required, Validators.min(0.01)]],
    delivery_days: [30, [Validators.required, Validators.min(1)]],
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<SendArchitectureQuoteDialogComponent, boolean>,
    private cdr: ChangeDetectorRef,
    @Inject(MAT_DIALOG_DATA) public data: SendArchitectureQuoteDialogData
  ) {}
  ...
  submit(): void {
    ...
    const validUntil = v.valid_until instanceof Date
      ? v.valid_until.toISOString().slice(0, 10)
      : undefined;
